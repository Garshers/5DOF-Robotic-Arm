...
def inverse_kinematics(R, Z, th1_base, phi_deg=0.0, elbow_up=True, reverse_base=False):
    ...
    Argumenty:
    R, Z: Współrzędne celu (mm) w płaszczyźnie R-Z
    ...
    # Kąt th1 jest już obliczony i dostarczony jako th1_base
    th1 = th1_base
...
def main():
    ...
    # Obliczenie R i Z (wspólne dla wszystkich)
    R_target = math.sqrt(x_target**2 + y_target**2)
    Z_target = z_target

    # Obliczenie th1 dla Bazy Normalnej
    th1_normal = 0.0 if R_target < 0.01 else math.atan2(y_target, x_target)
    
    # Obliczenie th1 dla Bazy Odwróconej (z normalizacją)
    ...
    # --- OBLICZANIE KINEMATYKI ODWROTNEJ (Krok 2: Solver 2D) ---
    ...
    print(f"\n-> Wybrano konfigurację: {best_name} ({best_cost:.4f})")
